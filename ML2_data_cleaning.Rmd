...
if(!any(ip%in%"intellidate")){
  devtools::install_github("ramnathv/intellidate")
}
library(intellidate)
library(plyr)
library(rio)
...
# Use local files?
useLocal <- params$useLocalKeyTables

# Compute Savani? 
# SLOW PROCESS!!!
computeSavani <- params$computeSavani

# Keytable directory
dir.keyTable <- file.path(params$osfdataPath,"!!KeyTables")

# Save directory
dir.saveData <- file.path(params$osfdataPath,"!!RawData")

# manylabRs sourceable function library
#devtools::source_url("https://raw.githubusercontent.com/ManyLabsOpenScience/ManyLabs2/master/Script%20-%20Sourceable%20function%20library/manylabRs_SOURCE.R")
source('~/Documents/GitHub/ManyLabs2/Script - Sourceable function library/manylabRs_SOURCE.R')
...
#in.IT(c("RCurl", "httr", "plyr", "reshape2", "tidyverse","rio"))

if(useLocal){
  
SourceInfoTable <- tbl_df(read.csv(file.path(dir.keyTable,"ML2_SourceInfo - ML2_SourceInfo.csv"),stringsAsFactors=FALSE))
SourceTable     <- tbl_df(read.csv(file.path(dir.keyTable,"ML2_DataInfoFilters - SourceNames.csv"),stringsAsFactors=FALSE))
FileNameTable   <- tbl_df(read.csv(file.path(dir.keyTable,"ML2_FileNameFilters - FileNames.csv"),stringsAsFactors=FALSE))
lookup          <- tbl_df(read.csv(file.path(dir.keyTable,"zavani_unique_values.csv"),header = TRUE,stringsAsFactors=FALSE))
chartable       <- tbl_df(read.csv(file.path(dir.keyTable,"zhong_text - Sheet2.csv"),header = TRUE,stringsAsFactors=FALSE))
...
lookup <- get.GoogleSheet(url = 'https://docs.google.com/spreadsheets/d/1aJJcCk8UvefbSAIePhmh6kEolOc5RwwB3cC_hBtrH5c/export?format=csv',skip = 4)$df
...
ExpSurvey <- read.csv("~/Documents/GitHub/ManyLabs2/Data/Raw to clean data/Raw Data/ML2_Experimenter_Survey.data.edited.formerge.csv")
ExpSurvey <- ExpSurvey[-1, ]

# # Code used to generate SourceInfoTable 
# for(s in seq_along(SourceInfoTable$Source)){ ID <-
...
#de-identify
if(deIDENTIFY){
  prefix <- ("~/Documents/GitHub/ManyLabs2/Data/Raw to clean data/Raw Data After Cleaning/")
...
# 'sex' is required for Risen.2
  ...
  prefix <- ("~/Documents/GitHub/ManyLabs2/Data/Raw to clean data/Raw Data After Cleaning PRIVATE/")
}
